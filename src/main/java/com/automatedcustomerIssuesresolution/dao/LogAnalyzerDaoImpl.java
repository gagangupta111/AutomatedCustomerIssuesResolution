...
public class LogAnalyzerDaoImpl implements LogAnalyzerDao{
    ...
    public List<String> getNonOptionArgs() {
        return getAppArgs().getNonOptionArgs();
    }

    public ApplicationArguments getAppArgs() {
        return appArgs;
    }
    ...
    public static String getRulesXMLPath(){
        return Constants.RULES_XML;
    }

    private void populateXMLRules() throws Exception{

        XMLRulesParser xmlRulesParser = new XMLRulesParser();
        rules = xmlRulesParser.readConfig(getRulesXMLPath());
    ...
    private void populateLogs() throws Exception {
        ...
        logFilesPath = getAppArgs().getNonOptionArgs().get(0);
        try {
            aroundDate = getAppArgs().getNonOptionArgs().get(1) + " " +  getAppArgs().getNonOptionArgs().get(2) + " " + getAppArgs().getNonOptionArgs().get(3);
        }catch (Exception e){
        ...
        Long millisecs = null;
        if (getAppArgs().getNonOptionArgs().size() > 4){
            millisecs = Long.parseLong(getAppArgs().getNonOptionArgs().get(4));
        }else {
            millisecs = range;
        }
